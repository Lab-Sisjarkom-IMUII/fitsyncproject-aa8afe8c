...
export async function POST(request: NextRequest) {
  try {
    ...
    const requiredFields = ['userId', 'weekData'];
    ...
    const user = {
      ...
      recentActivity: body.recentActivity || 'regular activity',
      activityLevel: body.activityLevel || 3
    };

    // The week data contains the information we want to analyze
    const datapoints = body.weekData;

    // Generate the prompt using our template
    const prompt = prompts.weeklySummary(user, datapoints);
    ...
    try {
      ...
      return NextResponse.json(
        { 
          error: "Failed to parse weekly summary from AI response",
